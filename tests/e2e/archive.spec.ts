...
test('archive searches, filters, and switches view', async ({ page }) => {
  ...
  await expect(page.locator('[data-recipe]:visible')).toHaveCount(20);
  await expect(page.locator('.search-field .ph-magnifying-glass')).toBeVisible();
  await expect(page.locator('.archive-arrow .ph-arrow-up-right')).toHaveCount(20);
  ...
  await page.getByRole('button', { name: /Pasta 7/ }).click();
  await expect(page.locator('[data-recipe]:visible')).toHaveCount(7);
